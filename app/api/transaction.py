...
def deposit(deposit_data: DepositCreate, db: DataBase = Depends(get_database)):
    db.deposit(deposit_data)
    return {"message": "Deposit successfully"}

@router.post("/transaction/withdraw")
def withdraw(withdraw_data: WithdrawCreate, db: DataBase = Depends(get_database)):
    try:
...
def transfer(transfer_data: TransferCreate, db: DataBase = Depends(get_database)):
