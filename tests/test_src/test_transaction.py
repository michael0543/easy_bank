...
def test_gen_insert_deposit_sql(setup_postgres):
    _account_id = 1
    _amount = 1000
    result = gen_insert_deposit_sql(
        account_id=_account_id, amount=_amount
    ).as_string(setup_postgres)
...
def test_gen_insert_withdraw_sql(setup_postgres):
    _account_id = 2
    _amount = 2000
    result = gen_insert_withdraw_sql(
        account_id=_account_id, amount=_amount
    ).as_string(setup_postgres)
    expect = "INSERT INTO transaction(account_id, type, amount, timestamp) "\
...
def test_gen_insert_transfer_sql(
    _account_id, _amount, _trans_type, expect, setup_postgres
):
    result = gen_insert_transfer_sql(
        account_id=_account_id, amount=_amount, trans_type=_trans_type
    ).as_string(setup_postgres)
    assert result == expect
