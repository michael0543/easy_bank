...
def test_gen_get_account_balance_sql(setup_postgres):
    _id = 1
    result = gen_get_account_balance_sql(
        account_id=_id
...
def test_gen_insert_account_sql(setup_postgres):
    _name = "Magic Johnson"
    _balance = 3200
    result = gen_insert_account_sql(
        name=_name, balance=_balance
    ).as_string(setup_postgres)
    expect = "INSERT INTO account(name, balance) "\
        "VALUES ('Magic Johnson', 3200) "\
...
def test_gen_update_account_balance_sql(
    ...
    result = gen_update_account_balance_sql(
        account_id=_account_id, amount=_amount, type=_type
    ).as_string(setup_postgres)
    assert result == expect
